...
test("관리 창의 확인·설치 완료 상태를 메인 화면에 즉시 전달한다", async () => {
  ...
  assert.doesNotMatch(
    mainSource,
    /async function updateDxvk\(version\)[\s\S]{0,500}blocksUnsignedDxvk/,
  )
  assert.match(
    mainSource,
  ...
  assert.match(managerSource, /Windows 보안 정책이 일부 DXVK 버전을 차단할 수 있습니다/)
  assert.match(managerSource, /0xC0E90002가 나타나면 이전 버전을 선택하세요/)
  assert.match(managerSource, /securityNotice\.hidden = !status\.securityPolicy\?\.blocksUnsignedDxvk/)
