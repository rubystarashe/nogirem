...
test("고정 시점 블랙박스 추출 편집 창과 구간 remux가 연결된다", async () => {
  ...
  assert.match(editorScript, /function timelineTimeToMediaTime\(time\)/)
  assert.match(editorScript, /function mediaTimeToTimelineTime\(time\)/)
  assert.match(editorScript, /video\.currentTime = timelineTimeToMediaTime\(selectionStart\)/)
  assert.match(editorScript, /snappedTime >= selectionStart/)
  assert.match(editorScript, /playbackTimelineTime < selectionEnd - 0\.01/)
