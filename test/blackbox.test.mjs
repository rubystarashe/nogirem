...
test("고정 시점 블랙박스 추출 편집 창과 구간 remux가 연결된다", async () => {
  ...
  assert.match(editorScript, /let requestedTrackSeconds = 30/)
  assert.match(editorScript, /event\.code !== "Space"/)
  assert.match(editorScript, /if \(dragging && !guideMoved && event\) seekFromPointer\(event\)/)
  assert.match(mainSource, /return createBlackboxEditorTrack\(session, 30\)/)
