...
test("고정 시점 블랙박스 추출 편집 창과 구간 remux가 연결된다", async () => {
  ...
  assert.match(mainSource, /ensureBlackboxEditorTrackMedia/)
  ...
  assert.match(editorScript, /gapPolicyField\.hidden = !hasGap/)
  assert.match(editorScript, /gapStart < selectionEnd && gapEnd > selectionStart/)
