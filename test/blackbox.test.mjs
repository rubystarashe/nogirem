...
test("고정 시점 블랙박스 추출 편집 창과 구간 remux가 연결된다", async () => {
  ...
  assert.match(managerSource, /녹화 용량 한도[\s\S]*남은 용량이 1GB 이하[\s\S]*청크 저장 드라이브[\s\S]*class="storage-drive-select"[\s\S]*모든 드라이브 청크 정리/)
  ...
  assert.match(nativeSource, /storageSpace\.available <= Gigabyte/)
  assert.match(nativeSource, /남은 용량이 1GB 이하라 녹화를 중단했습니다/)
