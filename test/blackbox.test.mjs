...
test("고정 시점 블랙박스 추출 편집 창과 구간 remux가 연결된다", async () => {
  ...
  assert.match(managerSource, /녹화 저장 위치[\s\S]*class="storage-path-input"/)
  assert.match(managerSource, /녹화 용량 한도[\s\S]*여유 용량이 최소 1GB 남도록[\s\S]*오래된 녹화 청크를 자동으로 정리합니다[\s\S]*청크 저장 드라이브[\s\S]*class="storage-drive-select"[\s\S]*모든 드라이브 청크 정리/)
  ...
  assert.match(mainSource, /compact \? 940/)
  ...
  assert.match(editorSource, /class="track-end">편집 창을 연 시점/)
  assert.match(editorSource, /title="마우스 휠로 포인터 위치를 중심으로 시간대를 확대하거나 축소합니다"/)
  assert.match(editorScript, /let viewportStart = 0[\s\S]*let viewportDuration = 0/)
  assert.match(editorScript, /const pointedTime = viewportStart \+ pointerRatio \* viewportDuration/)
  assert.match(editorScript, /viewportStart = pointedTime - pointerRatio \* nextDuration/)
  assert.match(editorScript, /timeline\.addEventListener\("wheel", zoomTimelineFromWheel, \{ passive: false \}\)/)
  assert.match(editorScript, /const targetTime = viewportStart \+ ratio \* viewportDuration/)
  ...
  assert.match(nativeSource, /class GlobalShortcutHook/)
  assert.match(nativeSource, /SetWindowsHookExW\([\s\S]*WH_KEYBOARD_LL/)
  assert.match(nativeSource, /CallNextHookEx\(nullptr, code, message, parameter\)/)
  assert.match(nativeSource, /control->command == "shortcut"[\s\S]*applyShortcut/)
  assert.match(nativeSource, /shortcutHook\.consume\(\)[\s\S]*recording[\s\S]*isMabinogiForeground/)
  ...
  assert.doesNotMatch(nativeSource, /newestStarted - chunks_\.front\(\)\.started/)
  assert.match(nativeSource, /\.nogirem-ring-index/)
  assert.match(nativeSource, /cachedEntry->second\.size == size/)
  assert.match(nativeSource, /saveLocked\(\);/)
