...
test("고정 시점 블랙박스 추출 편집 창과 구간 remux가 연결된다", async () => {
  ...
  assert.match(editorScript, /video\.currentTime = selectionStart \+ selectionDuration/)
  ...
  assert.match(nativeSource, /bool firstAudioSample = true/)
  assert.match(nativeSource, /timestamp - firstAudioTime - defaultAudioDuration/)
