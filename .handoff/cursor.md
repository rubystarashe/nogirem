...
Last Updated: 2026-09-16 20:52

## Current Objective
0.4.0 오버레이 기능의 인터페이스를 먼저 확정하고 이후 별도 다운로드 모듈과 실제 화면 미러링을 구현한다.

## Current Status
- 앱 버전을 0.4.0으로 올리고 고급 기능에 `오버레이` 인터페이스를 추가했다. 게임 화면의 원하는 영역, 특히 스킬 슬롯을 지정해 쿨타임을 보기 쉬운 화면 중앙 근처에 복제하는 목적과 `스킬 슬롯 지정 → 쿨타임 표시` 흐름을 보여준다. 터보 키처럼 별도 모듈 다운로드 방식으로 제공할 예정임을 안내하며, 사용자 요청대로 이번 단계에서는 비활성 `준비 중` 버튼만 두고 다운로드·설치·캡처·표시 IPC와 실제 기능은 구현하지 않았다. 관련 회귀 테스트는 이 제한도 검사한다. 고급 기능·터보 키·시작 회귀 테스트 45개, Vite 0.4.0 빌드와 lint가 통과했다.
- DXVK 정책 안내 수정까지 포함한 0.3.15 Windows x64 NSIS 설치본을 재생성하고 [GitHub Release](https://github.com/rubystarashe/nogirem/releases/tag/v0.3.15)로 정식 공개했다. 남아 있던 elevated input guard·recorder helper는 각 control JSON의 정상 stop 명령으로 종료해 파일 잠금을 해제했다. 전체 Node 테스트 173개와 네이티브 helper 4종·Vite·코드 서명·NSIS 패키징이 통과했다. 최종 설치본은 97,409,784바이트·SHA-256 `3A99FD3B…1F6F74`, blockmap은 103,329바이트·`A2BBA312…693563`, `latest.yml`은 345바이트·`ABAAF4AA…70FCDE`, 터보 키 자산은 291,840바이트·`D9504362…16A857`이다. 네 자산은 모두 uploaded 상태이고 원격 digest·크기 및 공개 URL의 HTTP 200 응답이 로컬 결과와 일치한다.
...
- Alt+Enter용 `WH_KEYBOARD_LL`도 50ms 상태 loop에 설치하지 않는다. 전용 `GetMessage` 스레드에서 즉시 callback을 반환하고 Alt+Enter에 해당하는 Enter만 소비한다.
...
## Constraints / Rules
- 마우스 커서 크기 기능에 게임 프로세스 주입을 도입하지 않는다. 지원값은 `75·100·125·150·200`만 허용하고 100%는 커서 기능 비활성 상태로 취급한다. helper 종료과정과 마비노기 포커스 이탈 시 Windows 커서를 반드시 복원해야 한다.
...
## Pending Tasks
1. 오버레이 모듈의 배포 자산 형식·무결성 manifest·동의 화면을 설계하고 터보 키와 독립된 다운로드/제거 IPC를 구현한다.
2. 게임 화면에서 복제할 영역 선택 UI와 오버레이 표시 위치·크기·투명도 설정을 확정한다.
3. 캡처·미러링 helper를 구현하고 마비노기·안티치트·다중 모니터·DPI 환경에서 성능과 호환성을 검증한다.
...
## Known Issues
- 0.4.0 오버레이는 현재 인터페이스만 있으며 `준비 중` 버튼은 의도적으로 비활성이다. 모듈 다운로드·설치·화면 캡처·오버레이 표시 기능은 아직 없다.
...
## Key Files
- `web/App.svelte`: 고급 기능의 오버레이 목적·예시·별도 모듈 다운로드 예정 인터페이스
- `web/styles.css`: 오버레이 흐름 안내와 일반·paused 화면 스타일
- `test/overlay-interface.test.mjs`: 인터페이스 노출과 미구현 IPC 경계 회귀 테스트
- `native/input-guard-helper/main.cpp`: Alt+Enter 차단과 마비노기 포커스 기반 `CursorBaseSize` 적용·원래 값 복원을 함께 담당한다.
- `electron/main.mjs`: 두 입력 기능의 통합 설정, helper 수명주기와 시작 시 커서 복구를 관리한다.
- `web/App.svelte`: 고급 기능의 게임 마우스 커서 크기 선택 UI를 제공한다.
...
## Recent Changes
- 앱 버전을 0.4.0으로 올리고 고급 기능에 스킬 슬롯 영역을 화면 중앙 근처로 복제할 오버레이의 정적 인터페이스를 추가했다. 실제 모듈과 IPC는 후속 작업으로 분리했다.
...
0.4.0 개발 앱의 고급 기능 화면에서 오버레이 행의 문구·간격·정보 위계를 확인한 뒤 모듈 설치 인터페이스의 상세 요구사항을 확정한다.
